  ...
  return (
    <div className="relative w-full bg-transparent">
      ...
      >
        ...
        <div className="absolute bottom-1/4 right-0 w-[500px] h-[500px] rounded-full bg-blue-600/5 blur-[150px] pointer-events-none -z-10" />
        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col justify-between flex-grow">
                ...
                <button
                  ...
                  className={`px-3.5 sm:px-4 py-1.5 rounded-full text-xs sm:text-sm font-inter font-medium transition-all duration-300 ${
                    activeFilter === tab.id
                      ...
                      : 'bg-white text-slate-600 hover:text-slate-950 border border-slate-200 hover:border-slate-300 hover:bg-slate-50 shadow-sm'
                  }`}
                >
                  {tab.label}
                </button>
          ...
          <div className="w-full overflow-visible py-2 sm:py-4 my-auto">
            ...
            >
              {filteredProjects.map((project) => (
                <div
                  key={project.id}
                  className="project-card group relative flex-shrink-0 w-[300px] sm:w-[360px] md:w-[400px] lg:w-[430px] flex flex-col p-1.5 sm:p-2 rounded-[2rem] bg-slate-50 border border-slate-200 hover:border-accent-blue/50 shadow-[0_10px_30px_rgba(15,23,42,0.04)] hover:shadow-[0_20px_45px_rgba(37,99,235,0.12)] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] select-none"
                >
                  {/* Inner Core Container */}
                  <div className="rounded-[calc(2rem-0.375rem)] bg-white overflow-hidden flex flex-col flex-grow justify-between border border-slate-100 shadow-sm">
                    ...
                    <div className="relative w-full h-48 sm:h-56 overflow-hidden bg-slate-100">
                      ...
                      <div className="absolute top-3.5 left-3.5 z-10">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-inter font-semibold bg-white/90 backdrop-blur-md text-accent-blue border border-slate-200 shadow-sm">
                      ...
                      {/* Top Right Link Icon */}
                      <div className="absolute top-3.5 right-3.5 z-10 w-8 sm:w-9 h-8 sm:h-9 rounded-full bg-white/90 backdrop-blur-md border border-slate-200 flex items-center justify-center text-slate-700 group-hover:text-accent-blue group-hover:border-accent-blue/60 group-hover:bg-blue-50 transition-all duration-300 shadow-sm">
                    ...
                    <div className="p-5 sm:p-6 flex flex-col flex-grow justify-between">
                      <div>
                        <h3 className="font-sora font-bold text-lg sm:text-xl text-slate-900 group-hover:text-accent-blue transition-colors duration-300">
                          {project.title}
                        </h3>
                        <p className="mt-1 text-xs sm:text-sm font-semibold text-slate-800">
                          {project.headline}
                        </p>
                        <p className="mt-2 text-xs sm:text-sm text-slate-500 font-inter leading-relaxed line-clamp-3">
                          {project.description}
                        </p>
                      </div>
                      ...
                      <div className="mt-4 pt-3.5 border-t border-slate-100 grid grid-cols-3 gap-2">
                        {project.metrics.map((metric) => (
                          <div key={metric.label} className="text-left">
                            <div className="font-sora font-bold text-sm sm:text-base text-slate-900 group-hover:text-accent-blue transition-colors">
                              {metric.value}
                            </div>
                            <div className="text-[10px] sm:text-[11px] text-slate-500 font-inter font-medium truncate">
                              {metric.label}
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-4 pt-3.5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                        <div className="flex flex-wrap gap-1.5">
                          {project.tags.slice(0, 3).map((tag) => (
                            <span
                              key={tag}
                              className="px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-inter font-medium bg-slate-100 text-slate-700 border border-slate-200"
                            >
                              {tag}
                            </span>
                          ))}
                        ...
                        <a
                          href={project.link}
                          className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1 rounded-full bg-slate-100 group-hover:bg-accent-blue text-xs font-sora font-semibold text-slate-800 group-hover:text-white transition-all duration-300 shadow-sm"
                        >
                          <span>View Project</span>
                          <div className="w-5 h-5 rounded-full bg-white/60 group-hover:bg-white/20 flex items-center justify-center transition-transform group-hover:translate-x-0.5">
                            <ArrowRight className="w-3 h-3 text-slate-700 group-hover:text-white" />
                          </div>
                        </a>
                      </div>
                    </div>
          ...
          <div className="mt-4 sm:mt-6 pt-3 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            {/* Live Progress Bar & Counter */}
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <span 
                ref={progressTextRef}
                className="font-sora text-xs font-bold text-accent-blue min-w-[50px]"
              >
              ...
              <div className="w-32 sm:w-48 h-1.5 rounded-full bg-slate-200 overflow-hidden">
                <div 
                  ref={progressBarRef}
                  className="h-full bg-accent-blue rounded-full transition-all duration-100 ease-out shadow-sm"
                  style={{ width: '20%' }}
                />
              </div>
              <span className="text-[11px] text-slate-500 font-inter font-medium hidden sm:inline">
                Scroll to explore
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
              <a
                href="#contact"
                className="group inline-flex items-center justify-between gap-3 pl-6 pr-2 py-2 rounded-full bg-slate-950 hover:bg-accent-blue text-white font-sora font-semibold text-xs sm:text-sm tracking-tight shadow-md hover:shadow-lg active:scale-[0.98] transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]"
              >
